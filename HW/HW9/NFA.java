...
public class NFA {
    ...
    public boolean matches(String s) {
        ...
        // Use Linked List because HashMap cannot store duplicate values.
        // Create a Linked List that has a single element Start State.
        if ( _startState == null)
        {
            return false;
        }
        Set<State> working     = new HashSet<State>();
        working.add( _startState );
        Set<State>    tempSetW = null;
        Set<State>    tempSet  = null;
        Set<State> replaceSet  = null;
        ...
        while ( isUpdated )
        {
            ...
            tempSetW = new HashSet<State>();
            for ( State tempStateI : working )
            {
                tempSet = tempStateI.successors( EPSILON );

                for ( State tempStateJ : tempSet )
                {
                    if ( !tempSetW.contains( tempStateJ ) && ( !working.contains( tempStateJ ) ) )
                    {
                        tempSetW.add( tempStateJ );
                        isUpdated = true;
                    }
                }
                tempSetW.add( tempStateI );
            }
        ...
        for ( int i = 0; i < s.length(); i++ )
        {
            ...
            for ( State tempStateI : working )
            {
                tempSet = tempStateI.successors(s.charAt(i) );

                for ( State tempStateJ : tempSet )
                {
                    if ( !tempSetW.contains( tempStateJ ) )
                    {
                        tempSetW.add( tempStateJ );
                    }
                }
