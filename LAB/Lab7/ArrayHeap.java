...
public class ArrayHeap<T> {
    ...
    private void bubbleUp(int index) {
        ...
        for (int i = index; i > 0; i--) {
            if (getNode(i - 1) != null && getNode(i)._priority
                    < getNode(i - 1)._priority) {
                swap(i, i - 1);
            }
        }
    }
    ...
    private void bubbleDown(int index) {
        ...
        for (int i = index; i <= size(); i++) {
            if (getNode(i + 1) != null && getNode(i)._priority > getNode(i + 1)._priority) {
                swap(i, i + 1);
            }
        }
    }
    ...
    public T removeMin() {
        if (size() > 1) {
            swap(1, size());
            T temp = removeNode(size()).item();
            bubbleDown(1);
            return temp;
        } else if (size() == 1) {return removeNode(1).item();}
        return null;
    }
    ...
    public void changePriority(T item, double priority) {
        ...
        for (int i = 1; i < size() + 1; i++) {
            if (item.equals(getNode(i)._item)) {

                if (getNode(i)._priority < priority) {
